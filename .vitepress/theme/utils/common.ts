...
export const isRelativeUrl = (link: string) => /^(?!www\.|http[s]?:\/\/|[A-Za-z]:\\|\/\/).*/.test(link);
 ...
 * - //github.com
 */
export const isHttpUrl = (link: string) => /^(https?:)?\/\//.test(link);
...
export function truncateWithElipsis(string = "", length = 0, etc = "..."): string {
